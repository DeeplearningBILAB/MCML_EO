 ...
 *	distribution in multi-layered turbid media with 
 **	embedded object which is modified from the original MCML code
...
void LaunchPhotonObj(double, InputStruct *, PhotonStruct *, long);	/**	Included by Vijitha et al **/
...
void DoOneRun(short NumRuns, InputStruct *In_Ptr)
{
  ...
  do {
	...
	if (In_Ptr->objLayer == 0 || In_Ptr->objCode == 0) {		/** launch photon when there is no object **/
		...
		LaunchPhotonObj(out_parm.Rsp, In_Ptr, &photon, i_photon);
	}
